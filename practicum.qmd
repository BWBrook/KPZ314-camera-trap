...
> **TASK 1.1** In ≤ 50 words, explain why the data are encoded in this way.
...
raw |>
  ...
  dplyr::mutate(
    delta = as.numeric(difftime(datetime, dplyr::lag(datetime), units = "mins"))
  ) |>
  dplyr::filter(!is.na(delta) & delta > 0) |>
  ggplot2::ggplot(ggplot2::aes(x = delta)) +
  ggplot2::geom_histogram(bins = 60, na.rm = TRUE) +
  ggplot2::labs(
...
gamma_all_adj     # prints scalar (adjusted)
gamma_region_adj  # prints tibble (adjusted)
```
